...
namespace Progressio.Worker.Consumers
{
    ...
    {
        private readonly ILogger<CommentLikedConsumer> _logger;
        private readonly IConfiguration _configuration;

        private IConnection? _connection;
        private IChannel? _channel;

        private const string QueueName = "comment.liked";
        private const string DeadLetterQueue = "comment.liked.dlq";
        private const string NotificationQueue = "send_notification";

        private static readonly int[] RetryDelaysMs = [1000, 2000, 4000, 8000];

        public CommentLikedConsumer(
            ILogger<CommentLikedConsumer> logger,
     ...
     cancellationToken);
            ...
            _logger.LogInformation("CommentLikedConsumer started, listening on '{Queue}'", QueueName);
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            consumer.ReceivedAsync += async (_, ea) =>
            {
                ...
                _logger.LogInformation("CommentLikedConsumer received (attempt {Attempt}): {Json}", attempt + 1, json);
                ...
                {
                    _logger.LogError(ex, "Error processing comment liked message (attempt {Attempt})", attempt + 1);

                    if (attempt < RetryDelaysMs.Length)
                    {
                        int delayMs = RetryDelaysMs[attempt];
                        _logger.LogWarning("Retrying comment liked message in {DelayMs}ms (attempt {Next}/{Max})",
                    ...
                    {
                        _logger.LogError("Comment liked message moved to DLQ after {Max} attempts", RetryDelaysMs.Length + 1);
        ...
        private async Task ProcessAsync(CommentLikedMessage message, CancellationToken ct)
        {
            var notification = new SendNotificationMessage
            {
                UserId = message.CommentAuthorUserId,
                Title = "Someone liked your comment",
                Message = "Your comment received a new like.",
                NotificationType = "CommentLiked",
                RelatedEntityId = message.CommentId
            };
        ...
        private async Task PublishNotificationAsync(SendNotificationMessage notification, CancellationToken ct)
        {
            try
            {
                ...
                _logger.LogInformation("Comment liked notification published for User {UserId}", notification.UserId);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Failed to publish comment liked notification for User {UserId}", notification.UserId);
            }
        }
